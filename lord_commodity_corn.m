...
sugarWorldNorm = corn / mean(corn, 'omitnan');
...
% Initialize
consumption(1) = corn(1);
production(1) = corn(1);

% Run Simulation
for t = 2:T
    dY = 0.03; % Placeholder for GDP growth
    consumption(t) = consumptionECM(dY, consumption(t-1), 1.02^t, corn(t), params);
    production(t) = productionDynamics(corn, t, lags, coeffs);
end

% Forecast Prices
P_forecast = forecastPrice(corn, [1, 1, 1]);
...
legend('US Corn Price','consumption','production');
